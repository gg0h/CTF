...
this is actually quite simple, the missing characters from 
...
need to match the sha1 hash of the default username at certain positions, shown by this code
...
so simply find the hash of the username and rearrange to the given positions and we have the missing characters, we can confirm with check_key()

I have a dirty script to do this here
...
if __name__ == "__main__":
    hash = hashlib.sha256(b"SCHOFIELD").hexdigest()

    print(hash[4] + hash[5] + hash[3] + hash[6] + hash[2] + hash[7] + hash[1] + hash[8])
